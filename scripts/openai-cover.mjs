...
const TONE = {
  LIGHT_BG: "\nBackground tone: LIGHT. Clean bright desk with white or light wood surface. Bright natural window lighting, airy atmosphere, minimal soft shadows.",
...
function buildPromptFromProducts(products, category, backgroundTone, contentType) {
  ...
  switch (dominantType) {
    ...
    case "monitor":
      ...
      break;
    case "teclado":
      sceneDescription = "a mechanical gaming keyboard with per-key RGB backlighting. Close-up view, keys sharply in focus. Monitor glow in the softly blurred background with bokeh. The keyboard dominates the frame.";
      break;
    case "mouse":
      sceneDescription = "a precision gaming mouse with RGB lighting on a smooth mouse pad. Keyboard and monitor blurred in background with bokeh effect. The mouse is large, sharp and dominates the frame.";
      break;
    case "console":
      ...
      break;
    default:
      sceneDescription = "assorted gaming products arranged on a clean desk. Warm ambient lighting, gaming room background with soft bokeh effect. Products are large and dominate the frame.";
  }
...
function buildEditPrompt(products, category, backgroundTone, contentType) {
  ...
  switch (dominantType) {
    ...
    case "monitor":
      ...
      break;
    case "teclado":
      sceneDescription = "a gaming desk with per-key RGB backlighting. The keyboards should be displayed showing their layout, switches, and lighting.";
      break;
    case "mouse":
...
async function analyzeProductBrightness(imageBuffers) {
  ...
  for (const buf of imageBuffers) {
    try {
      ...
      // skip if sharp fails on a particular buffer
    }
  }
  if (count === 0) return null;
  const avg = totalLuminance / count;
  // avg < 128 → products are dark → use light background
  // avg >= 128 → products are light → use dark background
...
export async function gerarCapaOpenAI({ mlProducts, category, slug, contentType }) {
  ...
  const backgroundTone = await analyzeProductBrightness(images.map(i => i.buffer));
  if (backgroundTone) {
    log("INFO", `Produtos detectados como ${backgroundTone === "light" ? "escuros" : "claros"}, usando fundo ${backgroundTone === "light" ? "CLARO" : "ESCURO"} para contraste`);
  } else {
    log("INFO", "Nao foi possivel analisar luminosidade, usando fundo neutro");
  }
