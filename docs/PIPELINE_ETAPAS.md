...
**Em 13/08/2026** (regeneração do artigo "Melhores Jogos para PC em 2026", 40º): **estrutura de lista determinística** (heading-pai `## Os N Melhores ... em {ano}` + itens `###`, via `ensureListStructure`), **imagem sempre após o título da seção** (prompt + `repositionImageMarkers`), **grounding por Google para listas de games** (`games_candidates.mjs` → "CANDIDATOS OBRIGATÓRIOS" no prompt → gate P2), `normalizarAnosPreposicional` (protege "Cyberpunk 2077"/"RTX 2060"), `montarQueryPesquisa`, `buildInternalLinksBlock(excludeSlug)` e gate corretor no último retry do fluxo de chamada única. Artigo regenerado aprovado no gate de revisão e no `validar-artigo.mjs` (0 falhas).
...
| 6 | **Redação** | Carlos Redator | ✅ sim | ✅ | **10/10** | LLM segmentada (frontmatter+blurb por item+corpo) ou chamada única; 3 tentativas com feedback; montagem determinística em código | **Medição corrigida (12/08)**: check de preços em prosa exclui a tabela comparativa. **Corrigido (13/08)**: `ensureListStructure` garante heading-pai + itens `###` em listas de games sem produtos; gate corretor no último retry (description/tags). **Pendência**: `minWords` 650 default ≠ alvo da persona (700-1100) |
| 7 | **SEO** | Felipe Otimizador | ✅ sim | ✅ | **10/10** | checkTitle/keyword nos 40%, frontmatter, links internos, normalizar anos | **Corrigido (12/08)**: capa resolvida antes do hook; `fontesComUrl` conta os dois formatos. **Corrigido (12/08, B)**: bug de severidade — keyword ausente/clickbait eram sempre P2, agora P1. **Corrigido (13/08)**: `normalizarAnosPreposicional` no corpo (protege "Cyberpunk 2077"/"RTX 2060"); `montarQueryPesquisa` limpa a query; `buildInternalLinksBlock(excludeSlug)` evita self-link. **Atenção**: `fontesComUrl < 2` (P1) e `description < 120` (P1) agora BLOQUEIAM — risco de falso positivo |
| 8 | **Design / Imagens** | Lucas Designer | ✅ sim | ✅ | **10/10** | RAWG jogos, Tavily fallback, capa IA (OpenAI→Stability→fallbacks), thumbs de produto, injeção de marcadores | **Medição corrigida (12/08)**: `fs.existsSync` resolve contra `public/`. **Pendência**: check de `alt` é P3 e fraco (não garante por-imagem). **Corrigido (13/08)**: queda progressiva do nome no RAWG e no fallback Tavily (`progressiveGameQueries`), filtro de URLs frágeis (`isFragileImageUrl`), preferência de hosts estáveis e `AbortSignal.timeout` no `fetch`. **Corrigido (13/08, tarde)**: `repositionImageMarkers` coloca a imagem APÓS o título da seção (dentro do container certo) |
...
5. **Gate de estrutura de lista (NOVO, 13/08/2026)** — `ensureListStructure` (`gerar-artigo.mjs`): para lista/review de **games sem produtos**, insere o heading-pai `## Os N Melhores ... em {ano}` e rebaixa os itens para `###`. Sem isso os jogos viram tópicos soltos no TOC (que trata `##` como tópico e `###` como subtópico recolhível). É determinístico — não depende da LLM.
6. **Gate de candidatos / grounding (NOVO, 13/08/2026)** — `games_candidates.mjs` (Serper + reserva Tavily → LLM extrai títulos) + bloco "CANDIDATOS OBRIGATÓRIOS" no prompt. No `validate()`, item de lista que não casa com um candidato vira **P2** (soft) — regenera nas primeiras tentativas e só publica com ressalva se a LLM insistir. Listas de games deixam de usar jogos antigos como "melhores de {ano}".
7. **Posicionamento de imagem (NOVO, 13/08/2026)** — `repositionImageMarkers` e o prompt colocam o `[IMG:]` na linha **após** o título `##`/`###` (imagem dentro da própria seção, abaixo do título, acima do texto). Antes o marcador ia antes do título e a imagem caía no container da seção anterior.
...
## Ajustes de 13/08/2026 (estrutura de lista + grounding por Google + normalizador de anos)

- **Estrutura de lista (`ensureListStructure`)** — lista/review de games com 0 produtos: detecta a sequência inicial de `##`-itens (para em `Comparativo|Tabela|Veredito|FAQ|Fontes|...`), insere `## Os N Melhores {frase} em {ano}` (`buildGamesListHeading`) e rebaixa os itens para `###`. Roda antes do reposicionamento de imagens e das âncoras.
- **Imagem após o título** — prompt (`gerar-artigo.mjs` ~linha 3535) e `repositionImageMarkers` (~linha 1098) colocam o `[IMG:]` na linha **após** o `##`/`###` (imagem dentro da seção, abaixo do título, acima do texto). Match tolerante por token ("Cyberpunk 2026 Phantom Liberty" casa com "## Cyberpunk 2026 — A Redenção...").
- **Grounding por Google (`games_candidates.mjs`)** — Serper busca `melhores jogos de pc {ano}` (+ variantes), reserva Tavily, LLM extrai `[{titulo, mencoes, fontes}]`. Prompt ganha "CANDIDATOS OBRIGATÓRIOS"; `validate()` marca item fora como **P2**; `extractListItemTitles`/`tituloSemelhante` fazem o match (≥60% dos tokens).
- **`normalizarAnosPreposicional`** (`tempo.mjs`) — o corpo só tem ano reescrito após "de/em/para/até" (ex.: "jogos de 2024"→2026), protegendo "Cyberpunk 2077", "RTX 2060" etc. Título/description/heading da lista seguem com `normalizarAnos` (agressivo).
- **`montarQueryPesquisa`** — remove prefixo "melhores"/"os N melhores" e corta na vírgula ("melhores melhores jogos para pc, jogos gratis..." → "jogos para pc Brasil 2026").
- **`buildInternalLinksBlock(excludeSlug)`** — não oferece o próprio artigo como link interno em regeneração (evita self-link no "Continue Explorando").
- **Gate corretor no último retry** — no fluxo de chamada única, antes de `exit(1)` por hard, tenta `corrigirPeloGate` (description/tags/marcadores/seções vazias) e revalida; só aborta se ainda houver hard.
- **`validateSourceCoverage` com janela de anos** — fiscaliza só o "intervalo de claim" ([ano-3, ano+1] exceto o ano do artigo); ano de lançamento de jogo (2021) não gera P1 falso.
