...
|---|-------|--------|-----------|-----------|-------|-----------|------------------------------|
| 1 | **Disparo** | GitHub Actions (sem agente) | ✅ sim | — | sem score formal | cron 09:30 UTC → checkout → `npm ci` → `npm test` → roda gerador | **Sem falha conhecida** — o cron de hoje executou |
...
| 8 | **Design / Imagens** | Lucas Designer | ✅ sim | ✅ | **10/10** | RAWG jogos, Tavily fallback, capa IA (OpenAI→Stability→fallbacks), thumbs de produto, injeção de marcadores | **Medição corrigida (12/08)**: `fs.existsSync` resolve contra `public/`. **Pendência**: check de `alt` é P3 e fraco (não garante por-imagem) |
...
- [ ] **P2 — Etapa 5: fallback de tema** quando a categoria de *lista/review* aborta (tentar próximo keyword trending em vez de `exit(1)`).
...
- [ ] **P9 — Etapa 5: cadeiras ainda aborta no sourcing** (1 produto passou no piso, precisa 3) — teclados passou; investigar fallback de tema/categoria em vez de `exit(1)`.
